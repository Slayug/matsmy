...
function getMessages() {
  return Array.from(document.getElementsByClassName("post__body")).filter(
    (el) => el.id.indexOf("user-activity") === -1
  );
}
...
// wait some time to be sure DOM loaded, DOMContentLoaded is not enough
setTimeout(() => {

  // naive test to know if the current tab is a Mattermost instance
  const rootDiv = document.getElementById("root");
  if (rootDiv && rootDiv.children[0].id === "channel_view") {
